...
* floor(p.x / gridSize) + sin(t * .1) * .2) + sin(4. * p.x * .5 + time * 2.) * cos(p.y * .5 + time * 2.) * .05;
}
...
vec3 clownFishTexture(vec2 p) {
  ...
  float x = -.3 + sin(p.x * 2. - time * 2. + cos(p.y * 1.2));
...
vec3 shade(in vec2 p) {
  vec2 p00 = coords();
  vec2 p0 = p00 + vec2(sin(1. + time * .2) * sin(p00.x * 10. + time * .1) * .2, 0.);
